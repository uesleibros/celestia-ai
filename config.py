...
async def _adicionar_lore(ctx, *, prompt: str) -> None:
  if ctx.author.id == 764259870563631114 or ctx.author.guild_permissions.administrator:
    try:
      ...
      rp_historico.insert(index, {"role": "system", "content": f"O {ctx.author.name} adicionou coisas na sua memória relacionada a {prompt}. Você se sente meio confusa ao ser questionada sobre coisas do tipo, foram simplesmente colocados à força na sua cabeça, você sente dor de cabeça com isso, confusa, mas tenta se manter forte e entende e logo segue o que foi colocado, só não entende direito mas compreende."})
      await ctx.message.add_reaction("✅")
    except Exception:
      await ctx.message.add_reaction("❌")
  else:
